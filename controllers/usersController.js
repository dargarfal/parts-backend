...
exports.getOneUser = async (req, res, next) => {
  ...
  try {
    ...
    if(user !== null){
      if(req.userid === user._id.toString() || req.userRole === 'administrador'){
        res.status(200).json(user);
      }else{
        res.status(400).json({ msg: 'Este usuario no tiene acceso a estos datos'});
      }       
    }else{
...
exports.updateUser = async (req, res, next) => {
  ...
  if(userupdate){
    if((req.userRole === 'administrador') || (req.userid === userupdate.id)){
      ...
      res.status(400).json({ msg: 'El usuario no tiene permisos para realizar esta acción'})
    }
  }else{
    res.status(400).json({ msg: 'El usuario no exite'});
  }
